(() => {
  ...
  const OPEN_KEY = "MATTBEAR_PANEL_OPEN";
  ...
  window.MATTBEAR_SITEWIDE = {
    version: "sitewide-layer-2",
    artist: cfg.artist,
    movement: cfg.movement,
    page: cfg.page,
    pageViewsLocalOnly: views,
    shortcut: "Ctrl+Shift+M",
    note: "Static page settings and route panel."
  };
  ...
  .mb-site-ring{position:fixed;left:10px;bottom:10px;z-index:9998;display:flex;gap:7px;flex-wrap:wrap;max-width:min(720px,calc(100vw - 20px));pointer-events:none}
  .mb-site-ring a{pointer-events:auto;position:relative;overflow:hidden;text-decoration:none;color:var(--mb-text);border:1px solid var(--mb-line);background:rgba(5,5,8,.74);backdrop-filter:blur(12px) saturate(1.2);border-radius:999px;padding:8px 10px;font:950 10px/1 Bahnschrift,Arial,sans-serif;letter-spacing:.09em;text-transform:uppercase;box-shadow:0 0 16px rgba(53,231,255,.1),0 10px 28px rgba(0,0,0,.28);transition:.18s ease}
  .mb-site-ring a:after{content:"";position:absolute;inset:-2px;background:var(--mb-rainbow);background-size:240% 100%;opacity:.1;z-index:-1;animation:mbSiteRainbow 7s linear infinite}
  .mb-site-ring a:hover,.mb-site-ring a:focus-visible{border-color:rgba(53,231,255,.6);box-shadow:0 0 20px rgba(53,231,255,.24),0 0 38px rgba(255,79,210,.12);outline:none;transform:translateY(-1px)}
  ...
  .mb-admin-grid a,.mb-admin-copy,.mb-admin-close{display:flex;align-items:center;justify-content:center;min-height:35px;text-align:center;text-decoration:none;color:var(--mb-text);border:1px solid var(--mb-line);border-radius:10px;background:rgba(255,255,255,.052);font:900 10px/1.1 Bahnschrift,Arial,sans-serif;letter-spacing:.09em;text-transform:uppercase;padding:7px;cursor:pointer}
  .mb-admin-grid a:hover,.mb-admin-copy:hover,.mb-admin-close:hover{border-color:rgba(53,231,255,.56);box-shadow:0 0 16px rgba(53,231,255,.16)}
  ...
  @media(max-width:640px){.mb-site-ring{left:7px;bottom:54px;max-width:calc(100vw - 14px)}.mb-site-ring a{font-size:9px;padding:6px 7px}.mb-admin-toggle{right:7px;bottom:7px}.mb-admin-panel{right:7px;bottom:50px;width:calc(100vw - 14px)}}
  @media(prefers-reduced-motion:reduce){.mb-site-ring a:after,.mb-admin-toggle span,.mb-rainbow{animation:none!important}.mb-admin-panel.open{animation:none!important}}
  `;
  ...
  ring.innerHTML = `<a href="${cfg.home}">HOME</a><a href="${cfg.hub}">HiberNation</a><a href="${cfg.musicTools}">Music Tools</a><a href="${cfg.choppa}">The Choppa</a><a href="${cfg.arcade}">Arcade</a><a href="${cfg.examples}">Examples</a>`;
  document.body.appendChild(ring);

  const panel = document.createElement("aside");
  panel.className = "mb-admin-panel";
  panel.id = "mb-admin-panel";
  panel.setAttribute("aria-label", "MATTBEAR panel");
  panel.innerHTML = `<div class="mb-admin-head"><h2 class="mb-admin-title"><span class="mb-rainbow">MATTBEAR</span><br>Control Panel</h2><span class="mb-admin-pill">CTRL SHIFT M</span></div><p><strong>${cfg.page}</strong></p><p>${cfg.artist} is the brand / artist / maker. ${cfg.movement} is the movement layer behind it.</p><p class="mb-admin-note">Static route panel active. Settings are local to this browser until a real backend is wired.</p><div class="mb-admin-grid"><a href="${cfg.home}">HOME</a><a href="${cfg.hub}">HiberNation</a><a href="${cfg.musicTools}">Music Tools</a><a href="${cfg.choppa}">The Choppa</a><a href="${cfg.arcade}">Arcade</a><a href="${cfg.suno}">Suno</a><a href="${cfg.soundcloud}">SoundCloud</a><button class="mb-admin-copy" type="button">Copy Page JSON</button><button class="mb-admin-close" type="button">Close</button></div><div class="mb-admin-status"><div><span>Page</span><b>${cfg.section}</b></div><div><span>Local views</span><b>${views}</b></div><div><span>Shortcut</span><b>Ctrl+Shift+M</b></div></div>`;
  document.body.appendChild(panel);

  const toggle = document.createElement("button");
  toggle.className = "mb-admin-toggle";
  toggle.type = "button";
  toggle.innerHTML = `<span>MB</span> Panel`;
