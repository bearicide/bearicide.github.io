(() => {
  ...
  window.MATTBEAR_SITEWIDE = {version:"shared-shell-4-title-boost",artist:cfg.artist,movement:cfg.movement,page:cfg.page,section:cfg.section,shareTitle,shareText,shareUrl,pageViewsLocalOnly:views,shortcut:"Ctrl+Shift+M"};
  ...
  .mb-site-ring a,.mb-site-ring button{pointer-events:auto;position:relative;overflow:hidden;text-decoration:none;color:var(--mb-text);border:1px solid var(--mb-line);background:rgba(5,5,8,.76);backdrop-filter:blur(12px) saturate(1.2);border-radius:999px;padding:8px 10px;font:1000 10px/1 Bahnschrift,Arial,sans-serif;letter-spacing:.09em;text-transform:uppercase;box-shadow:0 0 16px rgba(53,231,255,.14),0 10px 28px rgba(0,0,0,.3);transition:.18s ease;cursor:pointer;text-shadow:0 1px 0 #000,0 0 12px rgba(53,231,255,.35)}
  .mb-site-ring a:after,.mb-site-ring button:after{content:"";position:absolute;inset:-2px;background:var(--mb-rainbow);background-size:240% 100%;opacity:.14;z-index:-1;animation:mbSiteRainbow 7s linear infinite}
  .mb-site-ring a:hover,.mb-site-ring a:focus-visible,.mb-site-ring button:hover,.mb-site-ring button:focus-visible{border-color:rgba(53,231,255,.68);box-shadow:0 0 22px rgba(53,231,255,.3),0 0 42px rgba(255,79,210,.16);outline:none;transform:translateY(-1px)}
  .mb-admin-toggle{position:fixed;right:10px;bottom:10px;z-index:10000;border:1px solid rgba(255,224,122,.58);border-radius:12px;background:linear-gradient(180deg,rgba(24,24,32,.97),rgba(5,5,8,.99));color:var(--mb-text);box-shadow:0 20px 54px rgba(0,0,0,.54),0 0 28px rgba(255,79,210,.18);padding:9px 10px;font:1000 11px/1 Bahnschrift,Arial,sans-serif;letter-spacing:.12em;text-transform:uppercase;cursor:pointer;opacity:.36;transition:.18s ease;text-shadow:0 1px 0 #000}
  .mb-admin-toggle:hover,.mb-admin-toggle:focus-visible{opacity:1;transform:translateY(-2px);outline:none}
  .mb-admin-toggle span,.mb-rainbow{background:var(--mb-rainbow);background-size:240% 100%;-webkit-background-clip:text;background-clip:text;color:transparent;animation:mbSiteRainbow 7s linear infinite;filter:drop-shadow(0 2px 0 #000) drop-shadow(0 0 12px rgba(53,231,255,.65)) drop-shadow(0 0 26px rgba(255,79,210,.5))}
  .mb-admin-panel{position:fixed;right:10px;bottom:54px;z-index:9999;width:min(430px,calc(100vw - 20px));max-height:75vh;overflow:auto;color:var(--mb-text);background:linear-gradient(180deg,rgba(18,18,26,.98),rgba(4,4,8,.98));border:1px solid rgba(255,224,122,.44);border-radius:18px;box-shadow:0 26px 72px rgba(0,0,0,.62),0 0 38px rgba(53,231,255,.18),0 0 72px rgba(255,79,210,.1);padding:14px;display:none}
  .mb-admin-panel.open{display:block;animation:mbAdminRise .18s ease-out}
  .mb-admin-head{display:flex;align-items:center;justify-content:space-between;gap:8px;border-bottom:1px solid var(--mb-line);padding-bottom:10px;margin-bottom:10px}.mb-admin-title{font:1000 17px/.95 Bahnschrift,Arial,sans-serif;text-transform:uppercase;letter-spacing:.08em;margin:0}.mb-admin-pill{border:1px solid rgba(103,255,84,.32);color:var(--mb-green);border-radius:999px;padding:4px 7px;font:900 9px/1 Bahnschrift,Arial,sans-serif;text-transform:uppercase;letter-spacing:.09em;background:rgba(103,255,84,.07)}.mb-admin-panel p{color:var(--mb-muted);margin:8px 0;line-height:1.42;font:600 12px/1.42 Arial,sans-serif}.mb-admin-grid{display:grid;grid-template-columns:1fr 1fr;gap:7px;margin:10px 0}.mb-admin-grid a,.mb-admin-grid button{display:flex;align-items:center;justify-content:center;min-height:35px;text-align:center;text-decoration:none;color:var(--mb-text);border:1px solid var(--mb-line);border-radius:10px;background:rgba(255,255,255,.052);font:900 10px/1.1 Bahnschrift,Arial,sans-serif;letter-spacing:.09em;text-transform:uppercase;padding:7px;cursor:pointer}.mb-admin-grid a:hover,.mb-admin-grid button:hover{border-color:rgba(53,231,255,.56);box-shadow:0 0 16px rgba(53,231,255,.16)}.mb-admin-status{display:grid;gap:6px;margin-top:10px}.mb-admin-status div{display:flex;justify-content:space-between;gap:8px;border:1px solid rgba(255,255,255,.10);border-radius:9px;background:rgba(255,255,255,.04);padding:8px;color:var(--mb-muted);font:800 11px/1.1 Arial,sans-serif}.mb-admin-status b{color:var(--mb-gold);font-weight:1000;text-align:right}.mb-admin-note{border-left:3px solid var(--mb-green);background:rgba(103,255,84,.075);border-radius:0 8px 8px 0;padding:8px;color:var(--mb-text)!important}
  @keyframes mbSiteRainbow{0%{background-position:0% 50%}100%{background-position:240% 50%}}@keyframes mbAdminRise{from{opacity:0;transform:translateY(8px) scale(.98)}to{opacity:1;transform:none}}
  ...
  const flashShare = (text) => document.querySelectorAll(".mb-share-button").forEach(btn => { const old = btn.textContent; btn.textContent = text; setTimeout(() => { btn.textContent = old; }, 1200); });
  const sharePage = async () => {
    const payload = { title: shareTitle, text: shareText, url: shareUrl };
    try { if (navigator.share) await navigator.share(payload); else { await navigator.clipboard.writeText(shareUrl); flashShare("Copied URL"); } }
    catch (_) { try { await navigator.clipboard.writeText(shareUrl); flashShare("Copied URL"); } catch { console.log(shareUrl); flashShare("Logged URL"); } }
  };
  ...
  const setOpen = (open) => { panel.classList.toggle("open", open); toggle.setAttribute("aria-expanded", open ? "true" : "false"); try { localStorage.setItem(OPEN_KEY, open ? "1" : "0"); } catch (_) {} };
  ...
  document.addEventListener("keydown", event => { if (event.ctrlKey && event.shiftKey && event.key.toLowerCase() === "m") { event.preventDefault(); setOpen(!panel.classList.contains("open")); } if (event.key === "Escape" && panel.classList.contains("open")) setOpen(false); });
  panel.querySelector(".mb-admin-copy")?.addEventListener("click", async () => { const payload = JSON.stringify(window.MATTBEAR_SITEWIDE, null, 2); try { await navigator.clipboard.writeText(payload); flashShare("Copied JSON"); } catch (_) { console.log(payload); flashShare("Logged JSON"); } });
})();
